...
# n_tot because we simulate sampling with replacement)
...
print("This is a test to verify that the probabilities computed by " +
      "tourney_prob agree with the frequencies observed in a simulation.")
...
print("True selection probabilities: {}".format(p_sel2))
whist = whist / sum(whist)
plt.figure()
plt.plot(whist, label='Observed frequencies')
plt.plot(p_sel2, label='True probabilities')
plt.xlabel('$k$')
plt.ylabel('Selection Probability P($k$, {0}, {1})'.format(n, ts))
plt.legend()
plt.show(block=False)

print("Sample size = {}".format(n_trials))
print("This is like a repetition of the first test, but using a larger " +
      "number of samples")
print("---- Minimum prob = {}".format(min([p for p in p_sel2 if p > 0])))
...
print("---- Expected p is {}".format(p_mean))
print("---- Standard deviation of p is {}".format(p_std))
print("---- Average p is {}".format(np.mean(p_est)))

print("Now we test if E{1/(np)} is close to the sample avergage of 1/(n p).")
print("E{1/p} should be exactly one.")
ip_mean = sum(1.0/n for p in p_sel2)
ip_std = np.sqrt(sum(((1.0 / (n*p) - ip_mean)**2)*p for p in p_sel2))
print("---- Expected 1/(np) is {}".format(ip_mean))
print("---- Std of 1/(np) is {}".format(ip_std))
print("---- Average 1/(np) is {}".format(np.mean(1.0 / (n*p_est))))
print("Very likely, you will notice that the std is huge, despite the " +
      "average 1/(np) is not so large. This is because, with very low " +
      "probability, 1/(np) can be very large.")

print("\n*********************************************")
print("* SECOND TEST: tourney_prob PROBABILITIES ***")

print("The second test was aimed at computing the estimates based on joint " +
      "probabilities.")
print("It was used to verify that importance weights based on joint " +
      "probabilities are very bad (huge variance).")

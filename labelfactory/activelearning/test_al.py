...
print(('    Tamaño muestral: n = {0} muestras, etiquetadas en {1} lotes de ' +
       '{2} muestras cada uno.').format(ns, ns/pool_size, pool_size))
print('    Dimensión de los datos = {}'.format(X.shape[1]))
...
print(('Tamaño muestral: n = {0} muestras, etiquetadas en {1} lotes de {2} ' +
       'muestras cada uno.').format(ns, ns/pool_size, pool_size))

# ## Vector of succesive sample sizes
nq = list(range(pool_size, ns+pool_size, pool_size))
nq[-1] = min(nq[-1], ns)
n_pools = len(nq)

print('Dimension de la matriz de features = {}'.format(X.shape))
print('Numero de etiquetas = {}'.format(ns))
fig = plt.figure()
...
fig.suptitle('Testing algorithms')
...
fig.suptitle('Evolución del nº de errores con el nº de datos etiquetados')
plt.xlabel('Labeled dataset size')
...
fig.suptitle('Eficiencia muestral del Active Learning')
plt.xlabel('Demanda de etiquetas sin AL')
plt.ylabel('Demanda de etiquetas con AL')
